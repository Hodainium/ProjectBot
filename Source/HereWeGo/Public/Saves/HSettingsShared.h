...
#include "HSettingsShared.generated.h"
...
UENUM(BlueprintType)
enum class EHAllowBackgroundAudioSetting : uint8
{
	Off,
	AllSounds,
...
/**
 * UHSettingsShared - The "Shared" settings are stored as part of the USaveGame system, these settings are not machine
 ...
 */
UCLASS()
class HEREWEGO_API UHSettingsShared : public ULocalPlayerSaveGame
{

	GENERATED_BODY()

public:
	DECLARE_EVENT_OneParam(UHSettingsShared, FOnSettingChangedEvent, UHSettingsShared* Settings);
...
public:
	...
	static UHSettingsShared* CreateTemporarySettings(const UHLocalPlayer* LocalPlayer);

	/** Synchronously loads a settings object, this is not valid to call before login */
	static UHSettingsShared* LoadOrCreateSettings(const UHLocalPlayer* LocalPlayer);

	DECLARE_DELEGATE_OneParam(FOnSettingsLoadedEvent, UHSettingsShared* Settings);

	/** Starts an async load of the settings object, calls Delegate on completion */
	static bool AsyncLoadOrCreateSettings(const UHLocalPlayer* LocalPlayer, FOnSettingsLoadedEvent Delegate);
...
public:
	UFUNCTION()
	EHAllowBackgroundAudioSetting GetAllowAudioInBackgroundSetting() const { return AllowAudioInBackground; }
	UFUNCTION()
	void SetAllowAudioInBackgroundSetting(EHAllowBackgroundAudioSetting NewValue);

	void ApplyBackgroundAudioSettings();

private:
	UPROPERTY()
	EHAllowBackgroundAudioSetting AllowAudioInBackground = EHAllowBackgroundAudioSetting::Off;
...
public:
	UFUNCTION()
	EHGamepadSensitivity GetGamepadLookSensitivityPreset() const { return GamepadLookSensitivityPreset; }
	UFUNCTION()
	void SetLookSensitivityPreset(EHGamepadSensitivity NewValue) { ChangeValueAndDirty(GamepadLookSensitivityPreset, NewValue); ApplyInputSensitivity(); }

	UFUNCTION()
	EHGamepadSensitivity GetGamepadTargetingSensitivityPreset() const { return GamepadTargetingSensitivityPreset; }
	UFUNCTION()
	void SetGamepadTargetingSensitivityPreset(EHGamepadSensitivity NewValue) { ChangeValueAndDirty(GamepadTargetingSensitivityPreset, NewValue); ApplyInputSensitivity(); }

	void ApplyInputSensitivity();

private:
	UPROPERTY()
	EHGamepadSensitivity GamepadLookSensitivityPreset = EHGamepadSensitivity::Normal;
	UPROPERTY()
	EHGamepadSensitivity GamepadTargetingSensitivityPreset = EHGamepadSensitivity::Normal;
