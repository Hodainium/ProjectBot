...
#include "HInputModifiers.generated.h"


class UHAimSensitivityData;
/**
...
class UHSettingBasedScalar : public UInputModifier
{
	GENERATED_BODY()
...
/**
 * This is a deadzone input modifier that will have it's thresholds driven by what is in the H Shared game settings.
 */
UCLASS(NotBlueprintable, MinimalAPI, meta = (DisplayName = "H Settings Driven Dead Zone"))
class UHInputModifierDeadZone : public UInputModifier
{
	GENERATED_BODY()
...
UENUM()
enum class EHTargetingType : uint8
{
...
/** Applies a scalar modifier based on the current gamepad settings in H Shared game settings.  */
UCLASS(NotBlueprintable, MinimalAPI, meta = (DisplayName = "H Gamepad Sensitivity"))
class UHInputModifierGamepadSensitivity : public UInputModifier
{
	GENERATED_BODY()
public:
	...
	UPROPERTY(EditInstanceOnly, BlueprintReadWrite, Category = Settings, Config)
	EHTargetingType TargetingType = EHTargetingType::Normal;

	/** Asset that gives us access to the float scalar value being used for sensitivty */
	UPROPERTY(EditAnywhere, BlueprintReadOnly, meta = (AssetBundles = "Client,Server"))
	TObjectPtr<const UHAimSensitivityData> SensitivityLevelTable;

protected:
	virtual FInputActionValue ModifyRaw_Implementation(const UEnhancedPlayerInput* PlayerInput, FInputActionValue CurrentValue, float DeltaTime) override;
};

/** Applies an inversion of axis values based on a setting in the H Shared game settings */
UCLASS(NotBlueprintable, MinimalAPI, meta = (DisplayName = "H Aim Inversion Setting"))
class UHInputModifierAimInversion : public UInputModifier
{
	GENERATED_BODY()
