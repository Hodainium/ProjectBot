...
//#include UE_INLINE_GENERATED_CPP_BY_NAME(HTabListWidgetBase)

void UHTabListWidgetBase::NativeOnInitialized()
{
	Super::NativeOnInitialized();
}

void UHTabListWidgetBase::NativeConstruct()
{
...
void UHTabListWidgetBase::NativeDestruct()
{
	for (FHTabDescriptor& TabInfo : PreregisteredTabInfoArray)
	{
...
bool UHTabListWidgetBase::GetPreregisteredTabInfo(const FName TabNameId, FHTabDescriptor& OutTabInfo)
{
	const FHTabDescriptor* const FoundTabInfo = PreregisteredTabInfoArray.FindByPredicate([&](FHTabDescriptor& TabInfo) -> bool
	{
...
void UHTabListWidgetBase::SetTabHiddenState(FName TabNameId, bool bHidden)
{
	for (FHTabDescriptor& TabInfo : PreregisteredTabInfoArray)
	{
...
bool UHTabListWidgetBase::RegisterDynamicTab(const FHTabDescriptor& TabDescriptor)
{
...
void UHTabListWidgetBase::HandlePreLinkedSwitcherChanged()
{
	for (const FHTabDescriptor& TabInfo : PreregisteredTabInfoArray)
	{
...
void UHTabListWidgetBase::HandlePostLinkedSwitcherChanged()
{
...
void UHTabListWidgetBase::HandleTabCreation_Implementation(FName TabId, UCommonButtonBase* TabButton)
{
	FHTabDescriptor* TabInfoPtr = nullptr;

	FHTabDescriptor TabInfo;
	...
	if (TabButton->GetClass()->ImplementsInterface(UHTabButtonInterface::StaticClass()))
	{
		...
		{
			IHTabButtonInterface::Execute_SetTabLabelInfo(TabButton, *TabInfoPtr);
		}
	}
...
bool UHTabListWidgetBase::IsFirstTabActive() const
{
...
bool UHTabListWidgetBase::IsLastTabActive() const
{
	if (PreregisteredTabInfoArray.Num() > 0)
	{
...
bool UHTabListWidgetBase::IsTabVisible(FName TabId)
{
...
int32 UHTabListWidgetBase::GetVisibleTabCount()
{
	int32 Result = 0;
...
void UHTabListWidgetBase::SetupTabs()
{
	for (FHTabDescriptor& TabInfo : PreregisteredTabInfoArray)
	{
		if (TabInfo.bHidden)
