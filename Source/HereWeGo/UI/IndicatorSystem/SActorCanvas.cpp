...
#include "IActorIndicatorWidgetInterface.h"
...
EActiveTimerReturnType SActorCanvas::UpdateCanvas(double InCurrentTime, float InDeltaTime)
{
	...
	UHIndicatorManagerComponent* IndicatorComponent = IndicatorComponentPtr.Get();
	if (IndicatorComponent == nullptr)
	{
		IndicatorComponent = UHIndicatorManagerComponent::GetComponent(LocalPlayerContext.GetPlayerController());
