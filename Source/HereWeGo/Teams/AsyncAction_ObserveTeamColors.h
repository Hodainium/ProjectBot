...
class IHTeamAgentInterface;
class UHTeamDisplayAsset;
struct FFrame;

DECLARE_DYNAMIC_MULTICAST_DELEGATE_ThreeParams(FTeamColorObservedAsyncDelegate, bool, bTeamSet, int32, TeamId, const UHTeamDisplayAsset*, DisplayAsset);
...
public:
	...
	//  - For anything that can ever belong to a team (implements IHTeamAgentInterface),
...
private:
	void BroadcastChange(int32 NewTeam, const UHTeamDisplayAsset* DisplayAsset);

	UFUNCTION()
	void OnWatchedAgentChangedTeam(UObject* TeamAgent, int32 OldTeam, int32 NewTeam);

	UFUNCTION()
	void OnDisplayAssetChanged(const UHTeamDisplayAsset* DisplayAsset);

	TWeakInterfacePtr<IHTeamAgentInterface> TeamInterfacePtr;
