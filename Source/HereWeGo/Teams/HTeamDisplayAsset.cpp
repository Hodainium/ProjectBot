...
void UHTeamDisplayAsset::ApplyToMaterial(UMaterialInstanceDynamic* Material)
{
	if (Material)
	{
...
void UHTeamDisplayAsset::ApplyToMeshComponent(UMeshComponent* MeshComponent)
{
	if (MeshComponent)
	{
		for (const auto& KVP : ScalarParameters)
...
void UHTeamDisplayAsset::ApplyToNiagaraComponent(UNiagaraComponent* NiagaraComponent)
{
	if (NiagaraComponent)
	{
		for (const auto& KVP : ScalarParameters)
...
void UHTeamDisplayAsset::ApplyToActor(AActor* TargetActor, bool bIncludeChildActors)
{
...
#if WITH_EDITOR
void UHTeamDisplayAsset::PostEditChangeProperty(FPropertyChangedEvent& PropertyChangedEvent)
{
	Super::PostEditChangeProperty(PropertyChangedEvent);

	for (UHTeamSubsystem* TeamSubsystem : TObjectRange<UHTeamSubsystem>())
	{
