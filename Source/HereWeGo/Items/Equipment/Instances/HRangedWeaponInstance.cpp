...
#include "HRangedWeaponInstance.h"
...
UHRangedWeaponInstance::UHRangedWeaponInstance(const FObjectInitializer& ObjectInitializer)
...
void UHRangedWeaponInstance::PostLoad()
{
	Super::PostLoad();
...
#if WITH_EDITOR
void UHRangedWeaponInstance::PostEditChangeProperty(struct FPropertyChangedEvent& PropertyChangedEvent)
{
	Super::PostEditChangeProperty(PropertyChangedEvent);
	UpdateDebugVisualization();
}

void UHRangedWeaponInstance::UpdateDebugVisualization()
{
...
void UHRangedWeaponInstance::OnEquipped()
{
	Super::OnEquipped();
...
void UHRangedWeaponInstance::OnUnequipped()
{
	Super::OnUnequipped();
}

void UHRangedWeaponInstance::Tick(float DeltaSeconds)
{
	APawn* Pawn = GetPawn();
	check(Pawn != nullptr);
...
void UHRangedWeaponInstance::ComputeHeatRange(float& MinHeat, float& MaxHeat)
{
	float Min1;
	float Max1;
...
void UHRangedWeaponInstance::ComputeSpreadRange(float& MinSpread, float& MaxSpread)
{
	HeatToSpreadCurve.GetRichCurveConst()->GetValueRange(/*out*/ MinSpread, /*out*/ MaxSpread);
}

void UHRangedWeaponInstance::AddSpread()
{
...
float UHRangedWeaponInstance::GetDistanceAttenuation(float Distance, const FGameplayTagContainer* SourceTags, const FGameplayTagContainer* TargetTags) const
{
	const FRichCurve* Curve = DistanceDamageFalloff.GetRichCurveConst();
	return Curve->HasAnyData() ? Curve->Eval(Distance) : 1.0f;
}

float UHRangedWeaponInstance::GetPhysicalMaterialAttenuation(const UPhysicalMaterial* PhysicalMaterial, const FGameplayTagContainer* SourceTags, const FGameplayTagContainer* TargetTags) const
{
...
bool UHRangedWeaponInstance::UpdateSpread(float DeltaSeconds)
{
...
bool UHRangedWeaponInstance::UpdateMultipliers(float DeltaSeconds)
{
	...
	if (const UHCameraComponent* CameraComponent = UHCameraComponent::FindCameraComponent(Pawn))
	{
		...
		AimingAlpha = (TopCameraTag == H_Camera_Tags::TAG_Weapon_SteadyAimingCamera) ? TopCameraWeight : 0.0f;
	}
