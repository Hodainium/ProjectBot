...
namespace HConsoleVariables
{
	...
	static FAutoConsoleVariableRef CVarDrawBulletHits(
		TEXT("H.Weapon.DrawBulletHitDuration"),
	...
	static FAutoConsoleVariableRef CVarDrawBulletHitRadius(
		TEXT("H.Weapon.DrawBulletHitRadius"),
		DrawBulletHitRadius,
...
UHGameplayAbility_RangedWeapon::UHGameplayAbility_RangedWeapon(const FObjectInitializer& ObjectInitializer)
	: Super(ObjectInitializer)
{
	SourceBlockedTags.AddTag(TAG_WeaponFireBlocked);
}

UHRangedWeaponInstance* UHGameplayAbility_RangedWeapon::GetWeaponInstance() const
{
	return Cast<UHRangedWeaponInstance>(GetAssociatedEquipment());
}

bool UHGameplayAbility_RangedWeapon::CanActivateAbility(const FGameplayAbilitySpecHandle Handle, const FGameplayAbilityActorInfo* ActorInfo, const FGameplayTagContainer* SourceTags, const FGameplayTagContainer* TargetTags, FGameplayTagContainer* OptionalRelevantTags) const
{
	...
	if (bResult)
	{
		...
		{
			UE_LOG(LogHAbilitySystem, Error, TEXT("Weapon ability %s cannot be activated because there is no associated ranged weapon (equipment instance=%s but needs to be derived from %s)"),
				*GetPathName(),
				*GetPathNameSafe(GetAssociatedEquipment()),
				*UHRangedWeaponInstance::StaticClass()->GetName());
			bResult = false;
		}
	}
...
int32 UHGameplayAbility_RangedWeapon::FindFirstPawnHitResult(const TArray<FHitResult>& HitResults)
{
...
void UHGameplayAbility_RangedWeapon::AddAdditionalTraceIgnoreActors(FCollisionQueryParams& TraceParams) const
{
...
ECollisionChannel UHGameplayAbility_RangedWeapon::DetermineTraceChannel(FCollisionQueryParams& TraceParams, bool bIsSimulated) const
{
	return H_TraceChannel_Weapon;
}

FHitResult UHGameplayAbility_RangedWeapon::WeaponTrace(const FVector& StartTrace, const FVector& EndTrace, float SweepRadius, bool bIsSimulated, OUT TArray<FHitResult>& OutHitResults) const
{
...
FVector UHGameplayAbility_RangedWeapon::GetWeaponTargetingSourceLocation() const
{
...
FTransform UHGameplayAbility_RangedWeapon::GetTargetingTransform(APawn* SourcePawn, EHAbilityTargetingSource Source) const
{
	check(SourcePawn);
	AController* SourcePawnController = SourcePawn->GetController();
	UHWeaponStateComponent* WeaponStateComponent = (SourcePawnController != nullptr) ? SourcePawnController->FindComponentByClass<UHWeaponStateComponent>() : nullptr;

	// The caller should determine the transform without calling this if the mode is custom!
	check(Source != EHAbilityTargetingSource::Custom);
	...
	if ((Controller != nullptr) && ((Source == EHAbilityTargetingSource::CameraTowardsFocus) || (Source == EHAbilityTargetingSource::PawnTowardsFocus) || (Source == EHAbilityTargetingSource::WeaponTowardsFocus)))
	{
		...
		if (Source == EHAbilityTargetingSource::CameraTowardsFocus)
		{
	...
	if ((Source == EHAbilityTargetingSource::WeaponForward) || (Source == EHAbilityTargetingSource::WeaponTowardsFocus))
	{
		SourceLoc = GetWeaponTargetingSourceLocation();
	}
	else
	...
	if (bFoundFocus && ((Source == EHAbilityTargetingSource::PawnTowardsFocus) || (Source == EHAbilityTargetingSource::WeaponTowardsFocus)))
	{
...
FHitResult UHGameplayAbility_RangedWeapon::DoSingleBulletTrace(const FVector& StartTrace, const FVector& EndTrace, float SweepRadius, bool bIsSimulated, OUT TArray<FHitResult>& OutHits) const
{
#if ENABLE_DRAW_DEBUG
	if (HConsoleVariables::DrawBulletTracesDuration > 0.0f)
	{
		static float DebugThickness = 1.0f;
		DrawDebugLine(GetWorld(), StartTrace, EndTrace, FColor::Red, false, HConsoleVariables::DrawBulletTracesDuration, 0, DebugThickness);
	}
...
void UHGameplayAbility_RangedWeapon::PerformLocalTargeting(OUT TArray<FHitResult>& OutHits)
{
	APawn* const AvatarPawn = Cast<APawn>(GetAvatarActorFromActorInfo());

	UHRangedWeaponInstance* WeaponData = GetWeaponInstance();
	if (AvatarPawn && AvatarPawn->IsLocallyControlled() && WeaponData)
	{
		...
		const FTransform TargetTransform = GetTargetingTransform(AvatarPawn, EHAbilityTargetingSource::CameraTowardsFocus);
		...
		if (HConsoleVariables::DrawBulletTracesDuration > 0.0f)
		{
			...
			DrawDebugLine(GetWorld(), InputData.StartTrace, InputData.StartTrace + (InputData.AimDir * 100.0f), FColor::Yellow, false, HConsoleVariables::DrawBulletTracesDuration, 0, DebugThickness);
		}
#endif
...
void UHGameplayAbility_RangedWeapon::TraceBulletsInCartridge(const FRangedWeaponFiringInput& InputData, OUT TArray<FHitResult>& OutHits)
{
	UHRangedWeaponInstance* WeaponData = InputData.WeaponData;
	check(WeaponData);
	...
	for (int32 BulletIndex = 0; BulletIndex < BulletsPerCartridge; ++BulletIndex)
	{
		...
		{
#if ENABLE_DRAW_DEBUG
			if (HConsoleVariables::DrawBulletHitDuration > 0.0f)
			{
				DrawDebugPoint(GetWorld(), Impact.ImpactPoint, HConsoleVariables::DrawBulletHitRadius, FColor::Red, false, HConsoleVariables::DrawBulletHitRadius);
			}
#endif
...
void UHGameplayAbility_RangedWeapon::ActivateAbility(const FGameplayAbilitySpecHandle Handle, const FGameplayAbilityActorInfo* ActorInfo, const FGameplayAbilityActivationInfo ActivationInfo, const FGameplayEventData* TriggerEventData)
{
...
void UHGameplayAbility_RangedWeapon::EndAbility(const FGameplayAbilitySpecHandle Handle, const FGameplayAbilityActorInfo* ActorInfo, const FGameplayAbilityActivationInfo ActivationInfo, bool bReplicateEndAbility, bool bWasCancelled)
{
...
void UHGameplayAbility_RangedWeapon::OnTargetDataReadyCallback(const FGameplayAbilityTargetDataHandle& InData, FGameplayTag ApplicationTag)
{
	UAbilitySystemComponent* MyAbilityComponent = CurrentActorInfo->AbilitySystemComponent.Get();
	check(MyAbilityComponent);
	...
	if (const FGameplayAbilitySpec* AbilitySpec = MyAbilityComponent->FindAbilitySpecFromHandle(CurrentSpecHandle))
	{
		...
		if (!bProjectileWeapon)
		{
			if (AController* Controller = GetControllerFromActorInfo())
			{
				...
				{
					// Confirm hit markers
					if (UHWeaponStateComponent* WeaponStateComponent = Controller->FindComponentByClass<UHWeaponStateComponent>())
					{
		...
		{
			UE_LOG(LogHAbilitySystem, Warning, TEXT("Weapon ability %s failed to commit (bIsTargetDataValid=%d)"), *GetPathName(), bIsTargetDataValid ? 1 : 0);
			K2_EndAbility();
		}
	}
...
void UHGameplayAbility_RangedWeapon::StartRangedWeaponTargeting()
{
	...
	check(Controller);
	UHWeaponStateComponent* WeaponStateComponent = Controller->FindComponentByClass<UHWeaponStateComponent>();
	...
	if (FoundHits.Num() > 0)
	{
		...
		{
			FHGameplayAbilityTargetData_SingleTargetHit* NewTargetData = new FHGameplayAbilityTargetData_SingleTargetHit();
