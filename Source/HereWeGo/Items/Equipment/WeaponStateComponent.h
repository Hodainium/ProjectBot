...
struct FHScreenSpaceHitLocation
{
...
struct FHServerSideHitMarkerBatch
{
	FHServerSideHitMarkerBatch() { }

	FHServerSideHitMarkerBatch(uint8 InUniqueId) :
		UniqueId(InUniqueId)
	{ }

	TArray<FHScreenSpaceHitLocation> Markers;
...
UCLASS()
class UHWeaponStateComponent : public UControllerComponent
{
	GENERATED_BODY()

public:

	UHWeaponStateComponent(const FObjectInitializer& ObjectInitializer = FObjectInitializer::Get());
	...
	void GetLastWeaponDamageScreenLocations(TArray<FHScreenSpaceHitLocation>& WeaponDamageScreenLocations)
	{
...
private:
	...
	TArray<FHScreenSpaceHitLocation> LastWeaponDamageScreenLocations;

	/** The unconfirmed hits */
	TArray<FHServerSideHitMarkerBatch> UnconfirmedServerSideHitMarkers;
};
