...
#include "HGameplayAbility_RangedWeapon.generated.h"
...
UENUM(BlueprintType)
enum class EHAbilityTargetingSource : uint8
{
...
/**
 * UHGameplayAbility_RangedWeapon
 *
 * An ability granted by and associated with a ranged weapon instance
 */
UCLASS()
class UHGameplayAbility_RangedWeapon : public UHGameplayAbility_FromEquipment
{
	GENERATED_BODY()

public:

	UHGameplayAbility_RangedWeapon(const FObjectInitializer& ObjectInitializer = FObjectInitializer::Get());

	UFUNCTION(BlueprintCallable, Category = "H|Ability")
	UHRangedWeaponInstance* GetWeaponInstance() const;
...
protected:
	struct FRangedWeaponFiringInput
	{
		...
		UHRangedWeaponInstance* WeaponData = nullptr;
...
protected:
	...
	FTransform GetTargetingTransform(APawn* SourcePawn, EHAbilityTargetingSource Source) const;
