...
#include "HCameraModeThirdPerson.h"
...
namespace HCameraMode_ThirdPerson_Statics
{
	static const FName NAME_IgnoreCameraCollision = TEXT("IgnoreCameraCollision");
}

UHCameraModeThirdPerson::UHCameraModeThirdPerson()
{
	TargetOffsetCurve = nullptr;

	PenetrationAvoidanceFeelers.Add(FHPenetrationAvoidanceFeeler(FRotator(+00.0f, +00.0f, 0.0f), 1.00f, 1.00f, 14.f, 0));
	PenetrationAvoidanceFeelers.Add(FHPenetrationAvoidanceFeeler(FRotator(+00.0f, +16.0f, 0.0f), 0.75f, 0.75f, 00.f, 3));
	PenetrationAvoidanceFeelers.Add(FHPenetrationAvoidanceFeeler(FRotator(+00.0f, -16.0f, 0.0f), 0.75f, 0.75f, 00.f, 3));
	PenetrationAvoidanceFeelers.Add(FHPenetrationAvoidanceFeeler(FRotator(+00.0f, +32.0f, 0.0f), 0.50f, 0.50f, 00.f, 5));
	PenetrationAvoidanceFeelers.Add(FHPenetrationAvoidanceFeeler(FRotator(+00.0f, -32.0f, 0.0f), 0.50f, 0.50f, 00.f, 5));
	PenetrationAvoidanceFeelers.Add(FHPenetrationAvoidanceFeeler(FRotator(+20.0f, +00.0f, 0.0f), 1.00f, 1.00f, 00.f, 4));
	PenetrationAvoidanceFeelers.Add(FHPenetrationAvoidanceFeeler(FRotator(-20.0f, +00.0f, 0.0f), 0.50f, 0.50f, 00.f, 4));
}

void UHCameraModeThirdPerson::UpdateView(float DeltaTime)
{
...
void UHCameraModeThirdPerson::UpdateForTarget(float DeltaTime)
{
...
void UHCameraModeThirdPerson::DrawDebug(UCanvas* Canvas) const
{
...
void UHCameraModeThirdPerson::UpdatePreventPenetration(float DeltaTime)
{
	...
	IHCameraAssistInterface* TargetControllerAssist = Cast<IHCameraAssistInterface>(TargetController);

	IHCameraAssistInterface* TargetActorAssist = Cast<IHCameraAssistInterface>(TargetActor);
	...
	IHCameraAssistInterface* PPActorAssist = OptionalPPTarget.IsSet() ? Cast<IHCameraAssistInterface>(PPActor) : nullptr;
	...
	if (PPActorRootComponent)
	{
		...
		IHCameraAssistInterface* AssistArray[] = { TargetControllerAssist, TargetActorAssist, PPActorAssist };

		if (AimLineToDesiredPosBlockedPct < ReportPenetrationPercent)
		{
			for (IHCameraAssistInterface* Assist : AssistArray)
			{
				if (Assist)
				{
...
void UHCameraModeThirdPerson::PreventCameraPenetration(class AActor const& ViewTarget, FVector const& SafeLoc, FVector& CameraLoc, float const& DeltaTime, float& DistBlockedPct, bool bSingleRayOnly)
{
#if ENABLE_DRAW_DEBUG
	...
	//TODO IHCameraTarget.GetIgnoredActorsForCameraPentration();
	...
	{
		FHPenetrationAvoidanceFeeler& Feeler = PenetrationAvoidanceFeelers[RayIdx];
		if (Feeler.FramesUntilNextTrace <= 0)
		{
			...
			if (bHit && HitActor)
			{
				...
				if (HitActor->ActorHasTag(HCameraMode_ThirdPerson_Statics::NAME_IgnoreCameraCollision))
				{
					bIgnoreHit = true;
...
void UHCameraModeThirdPerson::SetTargetCrouchOffset(FVector NewTargetOffset)
{
...
void UHCameraModeThirdPerson::UpdateCrouchOffset(float DeltaTime)
{
