...
bool UHGridInventoryComponent::GetAllBlockingEntriesAtPointForEntry(const FHInventoryPoint& Point, UHGridEntry* RequestingEntry,
		...
		if (LocalGridArray->FindNextSlotPointForInstance(ItemInstance, Point))
		{
			...
			if (GridItemAtPoint)
			{
				if (GridItemAtPoint->CanStackWith(ItemInstance))
				{
					...
					if (StacksAdded > 0)
					{
						if (InventoryList.MarkItemIDDirty(GridItemAtPoint->GetLinkedID()))
						{
